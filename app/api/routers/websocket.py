...
async def employer_setup_websocket(websocket: WebSocket, role_id: str):
    ...
    try:
        ...
        async for message in pubsub.listen():
            ...
            if event_dict.get("type") in ("complete", "error") and event_dict.get("phase") == "db":
...
async def employee_setup_websocket(websocket: WebSocket, employee_id: str):
    """
    Subscribes to Redis channel:{employee_id} for employee onboarding events.
    """
    await websocket.accept()
    ...
    try:
        ...
        async for message in pubsub.listen():
            ...
            event_dict = json.loads(event_data_str)
            if event_dict.get("phase") == "db" and event_dict.get("type") in (
                "complete",
                "error",
            ):
                logger.info(
                    f"Received terminal db event '{event_dict.get('type')}' for {employee_id}. Closing WS."
                )
                break
