...
async def employer_setup_websocket(websocket: WebSocket, role_id: str):
    ...
    try:
        ...
        async for message in pubsub.listen():
            ...
            # Only the DB phase marks end of employer pipeline; normalization uses type "complete" too.
            if event_dict.get("phase") == "db" and event_dict.get("type") in ("complete", "error"):
                logger.info(
                    f"Received terminal db event '{event_dict.get('type')}' for {role_id}. Closing WS."
                )
                break
